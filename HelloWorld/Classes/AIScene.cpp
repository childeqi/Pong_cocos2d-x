...
Scene* AI::createScene()
{
    // 'scene' is an autorelease object
    auto scene = Scene::create();
    
    // 'layer' is an autorelease object
    auto layer = AI::create();
...
bool AI::init()
{
    ...
    //back
    auto back = MenuItemFont::create("Go Back", CC_CALLBACK_1(AI::GoBack, this));
    back->setPosition(origin + Vec2(visibleSize.width / 2, (visibleSize.height * 0.1)));
    // 4 soft buttons for UP and DOWN

    auto leftUp = MenuItemImage::create("square2.png", "square1.png", CC_CALLBACK_1(AI::leftUp, this));
    auto leftDown = MenuItemImage::create("square2.png", "square1.png", CC_CALLBACK_1(AI::leftDown, this));
    auto rightUp = MenuItemImage::create("square2.png", "square1.png", CC_CALLBACK_1(AI::rightUp, this));
    auto rightDown = MenuItemImage::create("square2.png", "square1.png", CC_CALLBACK_1(AI::rightDown, this));
...
//    CCLOG("ballposition x = %f, y = %f", ballSprite->getPosition().x, ballSprite->getPosition().y);
    ...
    if (ball_future.x >= (x_max - 40) && velocity.x > 0 )
    {
        if (ball_future.y > paddleRSprite->getPosition().y)
        {
...
bool AI::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
    CCLOG("onTouchBegan x = %f, y = %f", touch->getLocation().x, touch->getLocation().y);
}
void AI::leftUp(cocos2d::Ref *pSender)
{
    CCLOG("leftUp");
    auto action = MoveBy::create(0.2, Point(0, 10));
    paddleLSprite->runAction(action);
}
void AI::leftDown(cocos2d::Ref *pSender)
{
    CCLOG("leftDown");
    auto action = MoveBy::create(0.2, Point(0, -10));
    paddleLSprite->runAction(action);
}
void AI::rightUp(cocos2d::Ref *pSender)
{
    CCLOG("rightUp");
    auto action = MoveBy::create(0.2, Point(0, 10));
    paddleRSprite->runAction(action);
}
void AI::rightDown(cocos2d::Ref *pSender)
{
    CCLOG("rightDown");
    auto action = MoveBy::create(0.2, Point(0, -10));
    paddleRSprite->runAction(action);
...
void AI::GoBack(cocos2d::Ref *pSender)
{
...
void AI::menuCloseCallback(Ref* pSender)
{
